...
export function part1() {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (input[i][0].match(/NOT/)) {
            if (
                input[i][1].match(/[a-z]+/) &&
                typeof circuit[input[i][1]] === 'undefined'
            ) {
                circuit[input[i][1]] = 0x0000;
            }
            circuit[input[i][3]] = uint16(
                input[i][1].match(/[a-z]+/)
                    ? ~circuit[input[i][1]]
                    : ~parseInt(input[i][1]),
            );
            ...
            if (
                input[i][0].match(/[a-z]+/) &&
                typeof circuit[input[i][0]] === 'undefined'
            ) {
                circuit[input[i][0]] = 0x0000;
            }
            if (
                input[i][2].match(/[a-z]+/) &&
                typeof circuit[input[i][2]] === 'undefined'
            ) {
