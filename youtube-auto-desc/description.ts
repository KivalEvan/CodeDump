import { NoteJumpSpeed, round, secToMmss } from './deps.ts';
import {
   getSettings,
   HardwareSettings,
   ControllerOffsetSettings,
   CameraSettings,
} from './settings.ts';
...
export function getDescription(
   ...
   const songe = songList.find((e) =>
      e[1].toLowerCase().includes(bmData.title.toLowerCase())
   );
   if (!songe) {
      throw new Error(
...
Length: ${secToMmss(bmData.length)}
BPM: ${bmData.bpm}${
      bmData.events.bpm > 1 ? ` (${bmData.events.bpm} changes)` : ''
   }
NJS: ${njs.value}
Offset: ${njs.offset}
HJD: ${njs.hjd}
...
Obstacles: ${bmData.obstacles}${
      bmData.events.rotation
         ? '\nRotation Events: ' + bmData.events.rotation
         : ''
   }${bmData.events.njs ? '\nNJS Events: ' + bmData.events.njs : ''}

Basic Events: ${bmData.events.basic}
Color Boost Events: ${bmData.events.boost}
Event Box Groups:
...
The description is autogenerated on ${new Date().toISOString()} using modified Beat Saber Map Analysis dataset.`;
}
